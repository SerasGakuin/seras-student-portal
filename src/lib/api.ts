...
export const api = {
    ...
    analysis: {
        ...
        getMonthlyRanking: async (lineUserId: string | null | undefined, month: string, topN: number = 5) => {
            return fetchWithAuth<MonthlyRankingData>(
                `/api/analysis/ranking?month=${encodeURIComponent(month)}&topN=${topN}`,
                lineUserId
            );
        },
    },
