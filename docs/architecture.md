...
│   │   │   ├── cron/           # Cronジョブ (nightly, remind-open)
...
│   │   ├── analysisService.ts  # 在室状況分析（ヒートマップ・トレンド・日次内訳）
│   │   └── rankingAnalysisService.ts # 月間学習時間ランキング分析
│   │
...
│   │   ├── formatUtils.ts      # 表示フォーマットユーティリティ (分→時間変換)
│   │   ├── rankingUtils.ts     # ランキング計算 (Olympic方式タイ処理)
...
│       └── analysis.ts         # データ分析関連型 (ヒートマップ, ランキング等)
│
└── scripts/                # 開発・運用スクリプト
```
...
```
[UI] 分析ページで月選択 → 「PDF出力」ボタン押下
  ↓
[APIクライアント] api.analysis.getRanking({ month, topN }) を呼び出し
  ↓
[API Route] /api/analysis/ranking でリクエスト受信
  ├─ クエリパラメータ検証（month: YYYY-MM, topN: 1-50）
  ↓
[Service] rankingAnalysisService.getMonthlyRanking()
  ├─ 入退室記録 & 生徒マスターを取得
  ├─ 受験生の部 / 高2以下の部に分けて集計
  ├─ Olympic方式でランク付与（rankingUtils）
  ↓
[Response] MonthlyRankingData を返却
  ↓
[UI] RankingPdfView（A4固定レイアウト）を描画
  ├─ html-to-image でPNG化 (pixelRatio: 3)
  ├─ jsPDF でA4 PDFに変換
  ↓
[Download] PDFファイルがダウンロードされる
```
...
| `getMonthRangeJst(month)` | `"YYYY-MM"` から月初〜月末の JST 範囲を取得 |
| `countUniqueVisitDays(logs)` | ログ配列から JST 基準のユニーク通塾日数をカウント |
| `getUniqueVisitDaySet(logs)` | ログ配列から通塾日の `Set<string>` を取得 |
...
### E. 表示フォーマット (`formatUtils.ts`)

分数→時間表示の変換を共通化。`Math.floor(x/60)` を各コンポーネントで直接書かず、このユーティリティを使用します。

| 関数 | 用途 |
| :--- | :--- |
| `splitMinutes(totalMinutes)` | 分数を `{ hours, mins }` に分解（JSX で個別スタイリング用） |
| `formatMinutesHm(totalMinutes)` | `"2h 30m"` 形式の文字列を生成 |
| `formatMinutesHoursOnly(totalMinutes)` | `"2h"` 形式の文字列を生成（分を切り捨て） |

**使用箇所**: RankingPdfView, RankingWidget, RankingDetailView, ActivityHeatmap, badgeService 等

### F. ランキング計算 (`rankingUtils.ts`)

Olympic方式のタイ（同率）処理を共通化。同値の場合は同順位を付与し、次の順位をスキップします。

| 関数 | 用途 |
| :--- | :--- |
| `calculateRanksWithTies(items)` | Olympic方式でランク付与。例: `[100, 100, 80]` → `[1位, 1位, 3位]` |
| `getTopNWithTies(items, n)` | 上位N位以内を取得（タイで溢れた場合も全員含む） |

**使用箇所**: rankingAnalysisService（月間ランキング）

### G. キャッシュ設定 (`cacheConfig.ts`)
...
| `analysis.ts` | 分析関連（`OccupancyAnalysisData`, `MonthlyRankingData`, `TrendsPoint` 等） |
