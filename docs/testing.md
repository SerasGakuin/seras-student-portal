...
│   ├── exitTimeFillService.test.ts      # 退室時刻自動補完
│   ├── nightlyService.test.ts           # 夜間バッチ処理
│   ├── analysisService.test.ts          # 在室状況分析
│   └── rankingAnalysisService.test.ts   # 月間ランキング分析
├── lib/__tests__/
│   ├── apiHandler.test.ts               # APIハンドラー共通処理
│   ├── dateUtils.test.ts                # JST日付処理
│   ├── formatUtils.test.ts              # 表示フォーマット
│   ├── rankingUtils.test.ts             # ランキング計算（タイ処理）
│   └── stringUtils.test.ts             # 文字列処理
├── repositories/
...
└── __tests__/
    └── integration/
        ...
        ├── analysis-api.test.ts         # 在室状況分析API権限
        └── ranking-analysis-api.test.ts # 月間ランキング分析API権限
```
...
| **ExitTimeFillService** | ・退室時刻の自動補完ロジック<br>・個人/全体/デフォルト平均のフォールバック<br>・22:00上限のクリッピング |
| **NightlyService** | ・自動閉館と退室時刻補完の統合実行<br>・一方の失敗が他方に影響しないこと |
| **AnalysisService** | ・ヒートマップ集計（曜日×時間帯の平均値）<br>・トレンド計算（パーセンタイルバンド含む）<br>・日次内訳データの生成 |
| **RankingAnalysisService** | ・月間学習時間の集計<br>・受験生/非受験生のグループ分け<br>・Olympic方式タイ処理 |
...
| **Analysis API** | ・Guest: 401<br>・Student: 403<br>・Teacher/Principal: 200<br>・パラメータバリデーション (from/to 必須) |
| **Ranking Analysis API** | ・Guest: 401<br>・Student: 403<br>・Teacher/Principal: 200<br>・month パラメータバリデーション |
