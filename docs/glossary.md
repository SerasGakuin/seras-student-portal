...
**受験生**: `高3` または `既卒` の生徒。週間ランキング・月間ランキングで別グループ（受験生の部）として集計される。

## バッジ (Badges)

週間ランキングで付与される称号。受験生の部と高2以下の部で別々に集計。
...
| **ダッシュボード (Dashboard)** | 講師・教室長向けの統計・分析画面。月間ランキングPDF出力機能を含む |
| **データ分析 (Analysis)** | 在室状況のヒートマップ・トレンドグラフや月間ランキングを分析する画面 |
| **月間ランキング (Monthly Ranking)** | 月単位の学習時間を集計し、受験生の部/高2以下の部に分けて順位表示 |
| **ランキング (Ranking)** | 学習時間や通塾日数による生徒の順位表示 |
| **週間チャンピオン (Weekly Champions)** | 固定週間（月曜〜日曜）の各バッジ獲得者を表示するカード |
| **PDF出力** | 月間ランキングをA4 PDFとしてダウンロード（html-to-image + jsPDF） |
...
| **パーセンタイル** | データの分布位置を示す統計量。p25=下位25%, p75=上位25%の境界値 |
| **Olympic方式ランキング** | 同率は同順位を付与し、次順位をスキップする方式。例: 1位, 1位, 3位 |
| **退室時刻自動補完** | 毎日23:00 JSTに実行。退室忘れログに過去7日間の平均滞在時間で退室時刻を付与 |

## グループ名

| 用語 | 英語キー | 対象学年 |
| :--- | :--- | :--- |
| **受験生の部** | `examGroup` | 高3、既卒 |
| **高2以下の部** | `generalGroup` | 中1〜高2 |
