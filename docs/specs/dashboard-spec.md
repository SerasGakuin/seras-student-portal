...
#### 1. フィルターコマンドバー (`DashboardControls`)
画面上部に表示される操作パネルです。

**期間選択**:
- 今週 / 先週 / 今月 / 先月 / カスタム
- 月別アーカイブ（プルダウンから過去の月を選択）

**学年フィルター**:
- 全学年
- 高校生 / 中学生（グループ選択）
- 既卒 / 高3 / 高2 / 高1 / 中3 / 中2 / 中1（個別選択）
...
1.  **データ取得**:
    *   入退室ログ: `GoogleSheetOccupancyRepository.findAllLogs()`
    *   生徒マスター: `GoogleSheetStudentRepository.findAll()`

2.  **滞在時間計算**:
    *   `退室時刻 - 入室時刻` で算出します。
    *   **欠損値補完** (`getEffectiveExitTime`):
        *   `退室時刻` が存在する場合: そのまま使用
        *   `退室時刻` が未記入 & 入室から12時間未満: 現在時刻を仮の退室時刻として計算（在室中とみなす）
        *   `退室時刻` が未記入 & 入室から12時間以上: `MIN(入室+4時間, 当日22:00)` で補完（退室忘れとみなす）

3.  **通塾日数計算**:
    *   1日に複数回入退室しても、「1日」としてカウントします（Unique Date Count）。

4.  **オーバーラップ処理**:
    *   同一生徒の複数ログが重複する場合、マージして正確な滞在時間を算出します。
...
1. **集計期間**: 固定週間方式（月曜〜日曜）
   - 基準日が属する週の「先週」が対象
   - 開始日 = 先週の月曜 00:00:00 (JST)
   - 終了日 = 先週の日曜 23:59:59 (JST)

2. **グループ分け**:
   - **受験生の部 (exam)**: 高3・既卒
   - **高2以下の部 (general)**: 上記以外の在塾生
...
- ログインユーザーの属性に応じて「受験生の部」または「高2以下の部」を自動選択
- 講師・教室長は「受験生の部」をデフォルト表示
